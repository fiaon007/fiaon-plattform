...
| `server/routes.ts`, `server/storage.ts`, `server/routes/admin-users.ts`, `server/routes/founding.ts`, `server/index.ts` | ARAS-Plattform-Billing (anderes Produkt, gleiche Codebase) | Nicht Teil des FIAON-Antragflows | Unverändert |
| `server/routes/service-orders.ts` | ARAS-Plattform-Billing (Anrufpakete) | War nie eingehängt und nie übersetzbar | **Entfernt 09.09.2026** samt `db/migrations/006_service_orders.sql` |
...
### Update: Design-Revert + Kundendaten-Bearbeitung + Dubletten-System + Multi-Agent + Partner-Programm + Rechnungs-Fix (Pakete AA–AF)

**Paket AA — Revert des Interaktions-Feedback-Updates**
Kaputtes „plattformweites Interaktions-Feedback" per `git revert` zurückgerollt (KEIN Force-Push, Historie erhalten); funktionierende spätere Commits blieben unangetastet. Kernseiten (/admin/zahlungen, /admin/team, /agent, Antrag, Zahlungsseite) danach manuell verifiziert. Die Interaktions-Feedback-Idee wird nicht erneut versucht.

**Paket AC — Stammdaten-Korrektur (Agent + Admin)**
- Gemeinsamer Server-Helper `updateCustomerContact` (`fiaon-agent.ts`): editierbar sind NUR Vorname, Nachname, E-Mail, Telefon. Paket/Betrag/Zahlungsstatus/Referenz lehnt der Server hart ab (403) — unabhängig vom UI.
- Validierung: E-Mail-Format, Telefon-Normalisierung (+49…), Pflichtfelder. Jede Änderung schreibt einen Audit-Eintrag (Feld, alt → neu, Akteur, Zeit) in `fiaon_contact_log` → sichtbar in der Kunden-Timeline.
- E-Mail-Änderung: Webhooks/Mails nutzen immer den aktuellen Datensatz (verifiziert). Der Kunden-Login hängt an `email` → Änderung zieht das Login-Konto atomar mit (gleiche Zeile). Hat der Kunde bereits ein Passwort, liefert der Server `loginEmailChanged=true` → Hinweis-Dialog beim Agent („Kunde meldet sich künftig mit neuer E-Mail an").
- Dubletten-Warnung: Kollision der neuen E-Mail mit anderem Kunden → `duplicate`-Antwort mit Referenz, UI zeigt Warnung mit Verweis auf die Dubletten-Verwaltung (AD) statt stiller Doppelanlage.
- Endpoints: Agent `PATCH /agent/customers/:ref/contact-data`, Admin `POST /admin/applications/:ref/contact` (gleiche Audit-Pflicht).

**Paket AD — Dubletten: Erkennung, Wirkung, Verwaltung**
- **AD1 (Automatik)**: `supersedeSisterOrders` — wird eine Bestellung `paid`, werden ALLE offenen Bestellungen (pending_payment/claimed_paid) derselben E-Mail (case-insensitive) auf **`superseded`** gesetzt (Spalte `superseded_by` = bezahlte Referenz, Timeline-Eintrag). Superseded erzeugt KEINE Provision, verschwindet aus Reminder-Engine, Agent-Listen und Offen-Kacheln.
- **AD2 (doppelter Boden)**: Reminder-Engine, Bulk-Versand UND Agent-Mail schließen zusätzlich JEDE E-Mail mit irgendeiner `paid`-Bestellung per `NOT EXISTS`-Subquery aus. Admin-Override pro Bestellung: `allow_reminders_despite_paid` (Checkbox im Detail-Drawer, für echten Zweitkauf; Audit-Eintrag beim Umschalten).
- **AD3 (Werkzeuge)**: Dubletten-Verwaltung in /admin/zahlungen — Gruppierung per E-Mail (`GET /admin/duplicates/groups`), pro Gruppe alle Anträge mit Status, Aktion „Alle offenen stornieren". Einzelaktionen im Drawer: „Bestellung stornieren" (`cancelled`, stoppt alles, Provision-Clawback) und „Kunde löschen (DSGVO)" (Anonymisierung Name/E-Mail/Telefon/Adresse + KYC-PDF-Löschung; **Rechnungsdaten bleiben aus Buchhaltungspflicht erhalten**, Nummernkreis unangetastet). Beide mit Bestätigungsdialog + Audit.
- **Aufräumlauf** `POST /admin/duplicates/supersede-run`: wendet AD1 rückwirkend auf den Bestand an, loggt Zahlen (`[FIAON-DUBLETTE] Aufräumlauf: X superseded, Y bezahlte geprüft`), versendet KEINE Mails, idempotent. Neue Status-Badges „Ersetzt (Dublette)"/„Storniert" in Admin-UI.

**Paket AE — Multi-Agent + FIAON Partner-Programm**
- **AE1 Verteilung**: `distributeUnassignedOrders` — neue unzugewiesene pending/claimed-Bestellungen werden Round-Robin auf aktive Agents verteilt (Rotations-Zeiger `distribution_last_agent_id`). Einstellungen: global an/aus (`distribution_enabled`), Obergrenze offener Kunden pro Agent (`distribution_cap`, Default 50 — voller Agent wird übersprungen), pro Agent Schalter `distribution_active` (Drawer in /admin/team). Auto-Claim für Altbestände + manuelle Neuzuweisung bleiben.
- **AE2 Override**: Agent-Feld `recruited_by` (nur Admin/AE4-Flow). Bei jeder bestätigten Provision eines Geworbenen entsteht ein `kind='override'`-Eintrag für den Werber: Override-Satz (Default 5 % = `partner_override_bp`, pro Beziehung via `override_rate_bp` überschreibbar) × Kundenumsatz, eingefroren wie normale Provisionen. Anzeige getrennt als „Team-Umsatzbeteiligung". Storno der Kundenzahlung storniert den Override mit (bestehende Clawback-Mechanik). Overrides fließen ins selbe Guthaben/Auszahlungssystem.
- **HARTE REGEL (nicht verhandelbar, auch als Code-Kommentar in `fiaon-agent.ts`)**: Override gilt für **exakt EINE Ebene** — nur der direkte Werber erhält Override. Keine Ketten, keine Mehrfach-Ebenen. Rechtlicher Grund: Abgrenzung zu unzulässigen Vertriebssystemen.
- **AE3 Partner-Programm** (Sprachregelung: Partner/Partnerstatus/Meilensteine — NIE Level/Punkte/Stufe/Rang): 4 Status aus kumuliertem bestätigtem **EIGENumsatz** (Overrides zählen NICHT): Partner (Basis) → Senior Partner ab 25.000 € (+2 PP) → Executive Partner ab 75.000 € (+4 PP) → Managing Partner ab 200.000 € (+6 PP). Zuschlag wirkt nur auf NEUE Provisionseinträge (Einfrier-Prinzip). Schwellen/Zuschläge/Prämien Admin-editierbar (`partner_thresholds`, `partner_prizes`). Meilenstein-Erreichen → Zeile in `fiaon_partner_milestones` + Admin-Aufgabe „Prämie ausliefern" (offen/erledigt, /admin/team) + dezente Anzeige beim Agent. KEINE automatische Geldbuchung für Sachprämien. Agent-Seite `/agent/partner-programm` (monochrom, keine Gamification): Status, kumulierter Umsatz, Fortschritt zum nächsten Meilenstein, Vorteile, „Mein Team" (anonym aggregiert + Team-Umsatzbeteiligung).
- **AE4 Partner vorschlagen**: Formular auf der Partner-Seite → Anfrage in `fiaon_partner_suggestions` → Admin-Bereich „Partner-Anfragen" (/admin/team, offen/angenommen/abgelehnt). Annahme öffnet den normalen Einladungs-Flow mit vorbelegten Kandidatendaten, `recruited_by` wird automatisch auf den Vorschlagenden gesetzt. Ablehnung (optionaler Grund) informiert den Kandidaten NICHT. **KEINE Prämie für das Vorschlagen selbst** (bewusste Design-Entscheidung, Code-Kommentar) — der Werber profitiert ausschließlich über den umsatzbasierten Override.
- **Agent-Sichtbarkeit von Abschlüssen**: Bezahlte Kunden verschwinden aus der Anrufliste (`/agent/customers` liefert nur pending/claimed). Neue Sektion „Deine Abschlüsse" auf /agent zeigt WAS und WIE VIEL abgeschlossen wurde: Kundenname (Join auf `fiaon_applications`), Paket, Umsatz, Satz, Provisionsbetrag, Status — Override-Einträge mit Badge „Team-Beteiligung" und Summenausweis.

**Paket AF — Rechnungs-PDF repariert** (`fiaon-invoice.ts`, komplett neu gerendert)
A4 mit ≥20-mm-Rändern, FIAON-Kopf links + Rechnungsmeta rechtsbündig als Block, Empfänger im Fensterkuvert-Bereich, Leistungszeitraum unter der Beschreibung, Positionstabelle (Beschreibung | Zeitraum | Betrag) mit rechtsbündigen Beträgen und deutschem Zahlenformat, langer Paketname bricht sauber um, Summenblock rechts (fett), USt-Hinweis laut `INVOICE_VAT_MODE`, Zahlungsdaten-Block abgesetzt mit IBAN in Vierergruppen, einzeilige Fußzeile (FIAON LTD · Companies House No. 17318250 · Registered Office · Director) + Seitenzahl bei Mehrseitigkeit. Mit Extremwerten (60-Zeichen-Name, langes Paket) offline gerendert und pixelgeprüft.

**Drei nicht verhandelbare Regeln (in Code-Kommentaren verankert)**
1. Override strikt EINE Ebene. 2. Keine Rekrutierungs-Boni. 3. Partnerstatus nur aus Eigenumsatz (Overrides zählen nicht).
