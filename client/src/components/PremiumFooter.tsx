...
function Spalte({ titel, links, zu = (h) => h }: { titel: string; links: [string, string][]; zu?: (href: string) => string }) {
      ...
      <div className={`pf-falt${offen ? " auf" : ""}`}>
        <ul className="pf-falt-innen space-y-3.5 pt-1">
          {links.map(([href, label]) => (
            <li key={href + label}>
              <a href={zu(href)} className="pf-text text-[14px] transition-all duration-200 hover:translate-x-1 inline-block">{label}</a>
            </li>
          ))}
        </ul>
      </div>
...
// Englische Fußzeile (02.09.2026): dieselben Spalten aus shared/fiaon-sprache.ts —
// Ziel ist die englische Schwester, wo es sie gibt, sonst die deutsche Seite.
const EN_WISSEN = EN_FUSS.find((g) => g.titel === "Knowledge")?.links ?? [];
const EN_PLATTFORM = EN_FUSS.find((g) => g.titel === "Platform")?.links ?? [];
const EN_UNTERNEHMEN = EN_FUSS.find((g) => g.titel === "Company")?.links ?? [];
const EN_RECHT: [string, string][] = [...(EN_FUSS.find((g) => g.titel === "Legal")?.links ?? []), ["/cookie-einstellungen", "Cookie settings"]];
const DISCLAIMER_EN = "FIAON is a software-as-a-service (SaaS) and e-learning platform provided by FIAON LTD (128 City Road, London, EC1V 2NX, United Kingdom · Companies House No. 17318250). FIAON is not a credit institution or financial services provider and expressly does not give investment, tax or legal advice within the meaning of the German Banking Act (KWG). Nor do we broker credit or loans under the German Trade Regulation Act (in particular Section 34c GewO). We do not use affiliate tracking links. Where FIAON receives remuneration through partnerships with banks or other providers, this has no influence on how anything is presented; the decision on every financial product rests solely with the respective provider. All data, analyses, score simulations and dashboards generated by the software serve financial education only. Putting the strategies into practice and applying to financial institutions is entirely the user's own responsibility. The final decision on issuing a credit card or granting a specific credit limit rests 100 % with the respective financial institution. No guarantee, liability or warranty of a successful approval is given. The German version of all legal texts is binding.";

/** 19.09.2026: Im Business-Bereich steht die Fußzeile von FIAON Global (siehe GlassNav.tsx). */
export default function PremiumFooter({ bereich }: { bereich?: "business" } = {}) {
  const business = useBusinessBereich() || bereich === "business";
  return business ? <GlobalFuss /> : <PrivatFuss />;
}
...
function PrivatFuss() {
  ...
  const wissen = en ? EN_WISSEN : WISSEN;
  return (
    <footer className="relative overflow-hidden" style={{ background: "#0A0F1C" }}>
      ...
      <div className="max-w-[1280px] mx-auto px-6 py-16">
        ...
        {!/^\/(antrag|bonitaet-antrag|business\/start|business\/auftrag|en\/business\/start|en\/business\/auftrag|zahlung|termin|login|dashboard|mein-bereich)/.test(typeof window !== "undefined" ? window.location.pathname : "") && (
        <a href="/demo/kundenbereich" className="pf-demo" aria-label="Präsentation des Kundenbereichs öffnen">
          ...
          <span className="pf-demo-knopf">{ui.demoKnopf}<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M5 12h14M12 5l7 7-7 7" /></svg></span>
        </a>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-16 gap-y-2 md:gap-y-10 mb-10">
          ...
          <Spalte titel={ui.fussUnternehmen} links={en ? EN_UNTERNEHMEN : UNTERNEHMEN} zu={zu} />
          <Spalte titel={ui.fussRecht} links={en ? EN_RECHT : RECHTLICHES} zu={zu} />
        </div>
        ...
        <div className={`pf-wissen${wissenOffen ? " auf" : ""} mb-12`}>
          <button type="button" className="pf-wissen-kopf" aria-expanded={wissenOffen}
            ...
            <span className="pf-wissen-titel">
              <span role="heading" aria-level={3} className="pf-kopf text-[13px] font-medium uppercase tracking-[.15em]">{ui.fussWissen}</span>
              <span className="pf-wissen-zahl">{wissen.length} {ui.fussWissenZahl}</span>
            </span>
            <span className="pf-wissen-hinweis pf-leise text-[12px]">
              {wissenOffen ? ui.fussEinklappen : ui.fussAlle}
            </span>
        ...
        <div className="pt-8 mb-8" style={{ borderTop: "1px solid rgba(255, 255, 255, 0.1)" }}>
          {en ? <p className="pf-leise text-[12px] leading-relaxed">{DISCLAIMER_EN}</p> : (
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4" style={{ borderTop: "1px solid rgba(255, 255, 255, 0.05)" }}>
          ...
          <span className="pf-leise text-[12px]">
            © 2026 FIAON – FIAON LTD, Registered in England and Wales, Companies House No. 17318250 · Director: Justin Schwarzott. {en ? "All rights reserved." : "Alle Rechte vorbehalten. / All rights reserved."}
          </span>
          ...
          <div className="flex items-center gap-5">
            <a href="/agent"
               ...
               title={en ? "Access for FIAON staff" : "Zugang für FIAON-Mitarbeiter"}>
              ...
              {en ? "Staff access" : "Mitarbeiter-Zugang"}
            </a>
