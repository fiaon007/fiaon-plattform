...
export const KAPITEL_3: KapitelInhalt = {
  inhalte: {
    zeitleiste: {
      ...
      uebung: { art: "zeitleiste", stationen: [
        ...
        { tag: "Tag 0–2", titel: "Die erste Zahlung – immer direkt", wer: "Kunde", text: "Die erste Zahlung ist immer eine direkte Überweisung mit Zahlungsreferenz (Zahlungsdaten im Bereich und per E-Mail, QR-Code) – wie jede spätere Rate auch. Klickt er „Ich habe überwiesen“, wird er Stufe A – Beleg holen, nicht glauben.", system: "Kommt binnen 48 h kein Geld: automatische Erinnerung mit Terminlink; du siehst ihn unter „wartet auf Zahlung“." },
        { tag: "Tag 1–3", titel: "Kontoabgleich", wer: "Back-Office", text: "Der Kontoauszug (Banking Circle) wird täglich eingespielt, Zuordnungen bestätigt. Erst jetzt heißt es „Bezahlt“.", system: "Event `payment_confirmed` (einmalig, richtiger Betrag). Provision „rate“ entsteht für dich – bankbestätigt." },
        ...
        { tag: "Jeden Monat", titel: "Die Rate", wer: "System", text: "Am Monatstag der ersten Zahlung entsteht die nächste Rate – der Kunde überweist sie wie die erste, mit seiner Referenz. In der Akte steht, was dafür passiert ist.", system: "Abo-Motor: `rate_faellig` Tag −3 mit den Zahlungsdaten, Erinnerungen Tag 0/3/7." },
    ...
    zahlung: {
      ...
      bloecke: [
        schritte("So läuft eine Zahlung",
          ["Zahlungsdaten", "Jede Zahlung ist eine Überweisung mit Zahlungsreferenz – die erste wie jede Rate: Empfänger, IBAN, Verwendungszweck (Pflicht), QR-Code (EPC) zum Scannen, Kopieren-Knopf; die Zahlungsdaten stehen im Bereich und in jeder Zahlungsmail (aus der Akte: Zahlungsdaten senden). Das Geld landet direkt auf dem Geschäftskonto der FIAON LTD."],
          ["Der Kunde zahlt", "Eine Überweisung dauert in der Regel einen Bankarbeitstag. Klickt er „Ich habe überwiesen“, ist er Stufe A."],
          ["Kontoabgleich", "Das Back-Office spielt täglich den Kontoauszug (Banking Circle) ein und bestätigt Zuordnungen (Betrag + Verwendungszweck). Fehlt der Verwendungszweck, dauert es – deshalb ist er Pflicht."],
        ...
        ul(
          ...
          "Merksatz: Rate 1 bis 12 = Überweisung mit Referenz. Seit 19.09.2026 gibt es keine Lastschrift mehr: Jede Rate zahlt der Kunde per Überweisung (Daten in der Zahlungsmail und im Kundenbereich). Fragt ein Kunde nach einer Rückbuchung: Bereits per Lastschrift eingezogene Beträge erstattet FIAON; die Rate überweist er dann selbst.",
          "Bleibt eine Überweisung aus, erinnert der Zahlungsmotor (Tag 0/3/7) – ab Tag 14 bist du dran.",
    ...
    raten: {
      einleitung: "Der Zahlungsmotor ersetzt das Telefon-Inkasso. Er erinnert, mahnt – und sagt dir, wann du dran bist.",
      bloecke: [
        tab(["Tag", "Was passiert", "Wer"],
          ["−3", "`rate_faellig`: Hinweis mit Betrag, Referenz, Zahlungslink", "System"],
          ["0", "Rate wird fällig; die Überweisung wird erwartet", "System"],
          ["0 / 3 / 7", "`rate_erinnerung` per Mail, SMS, künftig WhatsApp – mit den Zahlungsdaten", "System"],
          ["14", "Du rufst an – weich, mit Entschuldigung (Reaktivierungs-Leitfaden). Zwei Wege: Zahlung erreichen (Altbestand: Reaktivierungsbonus 50 % des Zahlungswerts) oder die Rate einen Monat aussetzen (0 €, aber vorgestellt + Onboarding-Termin gebucht)", "Du"],
    ...
    provision: {
      ...
      bloecke: [
        tab(["Baustein", "Was du bekommst", "Wann"],
          ...
          ["Reaktivierungsbonus (nur Altbestand)", "50 % des Zahlungswerts, wenn ein Altkunde mit überfälliger Rate durch dein Gespräch zahlt – gilt für den Bestand von vor dem Office-Start, nicht für neue Kunden", "bei bankbestätigter Zahlung"],
  ...
  test: [
    ...
    frage("Welcher Schritt der Agenda räumt den häufigsten Streitfall aus?", ["Plattform-Tour", "Abo-Klarheit: Betrag, nächste Rate, Kündigungsweg – Antwort wörtlich notieren", "Begrüßung", "Unterlagen"], 1, "„Ich dachte, das war einmalig“ – dieser Schritt verhindert es."),
    ...
    frage("Wie zahlt der Kunde die ERSTE Rate?", ["Per PayPal", "Immer direkt per Überweisung mit Zahlungsreferenz – Zahlungsdaten im Bereich und per E-Mail", "Per Stripe", "Bar"], 1, "Erste Zahlung immer direkt – und jede weitere Rate ebenso per Überweisung."),
    frage("Ein überfälliger ALTKUNDE (Bestand von vor dem Office-Start) zahlt seine Rate nach deinem Anruf. Was bekommst du?", ["Nichts extra", "Den Reaktivierungsbonus: 50 % des Zahlungswerts – gilt nur für den Altbestand; bei neuen Kunden läuft die normale Provision", "Die ganze Rate", "10 €"], 1, "E-042a: Der 50-%-Bonus gilt nur für den Altbestand."),
