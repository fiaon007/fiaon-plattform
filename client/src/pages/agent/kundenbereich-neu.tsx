...
//
// 19.09.2026 (E-194): Zahlungswege nachgezogen. „Rate zahlen“ kennt nur noch
// die Überweisung (mit GiroCode); Sofortzahlung per Bank-App und automatischer
// Einzug sind mit dem Ende der Zusammenarbeit mit dem Zahlungsdienst entfallen.
// ═══════════════════════════════════════════════════════════════════════════
...
const KUNDE: { reiter: string; was: string; warum: string }[] = [
  ...
  { reiter: "Geld", was: "Nächste Rate, „Rate zahlen“ per Überweisung mit GiroCode, alle Raten mit Haken, der Monatsbericht zum Nachrechnen.", warum: "Jeder Monat hat einen Betrag, den der Kunde nachrechnen kann. Die Bankdaten kommen aus einer Quelle (shared/fiaon-bank.ts)." },
...
const TECHNIK: { bereich: string; details: string[] }[] = [
  ...
  { bereich: "Schalter (fiaon_settings)", details: ["app_brief_an (Brief-Weg), app_antraege_an (Vollmacht, Unterschrift, Anträge), app_bericht_mail (Monatsbericht per Mail) — alle über POST /admin/app/einstellung, ohne Deploy.", "Mitteilungen nur mit VAPID-Schlüsseln aus der Umgebung; ohne Schlüssel „nicht verfügbar“."] },
  { bereich: "Läufe (nur Produktion)", details: ["fristenwaechter (6 h), monatsbericht (täglich, wirkt am 1.–3.), app-ereignisse-aufraeumen (24 h, löscht nach 90 Tagen), push-rate-erinnerung (24 h, Nachtruhe 21–8 Uhr, eine Mitteilung je Tag).", "Keine Kundenmails aus Läufen — Mails nur über mailSenden() mit Vorlage im Mailwerk (app_login_link, app_monatsbericht)."] },
...
const ZEIT: { wann: string; was: string }[] = [
  ...
  { wann: "05.09., 22:50", was: "Teil 3: Hilfe und Termine. Teil 4: Rate zahlen (seit 19.09. nur noch per Überweisung mit GiroCode)." },
