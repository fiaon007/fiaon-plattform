...
export function mailHtml(b: MailBaustein): string {
  ...
  const abmelden = b.abmeldeUrl
    ? `<p style="margin:10px 0 0;font:400 12px/1.6 ${SCHRIFT};color:#9ca3af;">${b.du ? "Du möchtest diese Hinweise nicht mehr?" : r.abmeldenFrage} <a href="${b.abmeldeUrl}" style="color:#9ca3af;">${r.abmeldenLink}</a> — ${r.abmeldenSchluss}</p>` : "";
...
<body style="margin:0;padding:0;background:#f0f4fa;">
  ...
  <table role="presentation" cellpadding="0" cellspacing="0" width="100%" bgcolor="#f0f4fa" style="background-color:#f0f4fa;">
    <tr><td align="center" style="padding:30px 12px 44px;">
      ...
      ${b.persoenlich ? "" : `<p style="margin:18px 0 0;font:400 11px/1.5 ${SCHRIFT};color:#9aa7bd;">${b.du ? "Diese Nachricht wurde automatisch erstellt." : r.automatisch}</p>`}
    </td></tr>
  </table>
</body>
</html>`;
