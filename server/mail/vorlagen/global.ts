...
//   stichtag_text, anlass, email, sprache.
// ═══════════════════════════════════════════════════════════════════════════
...
/** Dieselbe Liste für die englische Startmail — gleiche Reihenfolge, gleicher Umfang. */
export const GLOBAL_UNTERLAGEN_EN: string[] = [
  "Passports of the shareholders and the managing directors (colour copy)",
  "Proof of address, no older than three months",
  "Shareholder list or commercial register extract of your company",
  "The name you would like for the US company, in three variants",
...
const PAARE: Record<string, Record<GlobalMailSprache, MailBaustein>> = {
  ...
  global_start: {
    ...
    en: {
      ...
      absaetze: [
        ...
        "How to send us the documents securely is something we discuss in the kick-off call.",
      ],
      daten: [
        { label: "Order", wert: "{{params.antrag_id}}" },
        { label: "Package", wert: "{{params.paket}}" },
  ...
  // Der Zugang zu „Mein Auftrag" — ein frischer, signierter Link je Auftrag
  // (server/lib/fiaon-global-zugang.ts). Geht raus, wenn der Kunde ihn selbst
  // anfordert (POST /global/zugang, abgelaufener Link), wenn ein Firmenkunde es
  // am Privatkunden-Login versucht, oder wenn die zuständige Person ihn aus dem
  // Office schickt. Ein Passwort gibt es für Firmenaufträge nicht.
  global_zugang: {
    de: {
      kopfSatz: KOPF, rechtsSatz: GLOBAL_ROLLEN.de.fiaon,
      betreff: "Ihr Link zu „Mein Auftrag“ — {{params.paket}}",
      preheader: "Stand, Vertrag, Rechnung und Unterlagen zu Ihrem Auftrag — ohne Passwort.",
      titel: "Ihr Link zu „Mein Auftrag“",
      absaetze: [
        "{{params.anrede_zeile}}, hier ist Ihr persönlicher Link zu „Mein Auftrag“ für <b>{{params.firma}}</b>. Dort sehen Sie den Stand Ihres Auftrags, Vertrag und Rechnung, Ihre Unterlagen und die nächsten Schritte.",
        "Der Link gehört zu genau diesem Auftrag und gilt dreißig Tage; ein Passwort brauchen Sie nicht. Bitte geben Sie ihn nur an Personen weiter, die Ihren Auftrag sehen dürfen.",
        "Sie haben diesen Link nicht angefordert? Dann ist nichts zu tun — ohne ihn öffnet niemand Ihren Auftrag.",
      ],
      daten: [
        { label: "Auftrag", wert: "{{params.antrag_id}}" },
        { label: "Paket", wert: "{{params.paket}}" },
    ...
    en: {
      sprache: "en", kopfSatz: KOPF, rechtsSatz: GLOBAL_ROLLEN.en.fiaon,
      betreff: "Your link to “My order” — {{params.paket}}",
      preheader: "Status, contract, invoice and documents for your order — no password needed.",
      titel: "Your link to “My order”",
      absaetze: [
        "{{params.anrede_zeile}}, here is your personal link to “My order” for <b>{{params.firma}}</b>. There you can see the status of your order, the contract and the invoice, your documents and the next steps.",
        "The link belongs to this order only and is valid for thirty days; you do not need a password. Please share it only with people who are allowed to see your order.",
        "You did not request this link? Then there is nothing to do — nobody can open your order without it.",
      ],
      daten: [
        { label: "Order", wert: "{{params.antrag_id}}" },
        { label: "Package", wert: "{{params.paket}}" },
